...
int DAQDevice::_DAQ_Start(int amplification, int timebaseIndex, int sampleInterval, DataHandler * dataHandler)
{
	// start data aquisition  and store input in an array.
	int status = DAQ_Start(DEVICE_AT_MIO_16, CHANNEL_0, amplification, (short*) dataHandler->getMemoryHandle(), MAX_MEMORY_ELEMENTS, timebaseIndex, sampleInterval);
	if (status != 0){
