...
void CDLTSdevice::start(CDLTSguiDoc * dltsDoc)
{
	...
	try {
		...
		long dataArraySum[RESULT_FIELD_SIZE];
		...
		// Meßschleife // Temperaturmessung*)
			...
			//for (int averageIdx = 0; averageIdx < dltsDoc->getNumberOfAveragings(); averageIdx++ ) {
				...
				m_pDAQDevice->_DAQ_Start(dltsDoc->getGain(), TIMEBASE_INDEX , SAMPLE_INTERVAL, m_pDataHandler);
				...
				{ //store)
					m_pDataHandler->copyData(dataArray, nullbalance);
			...
			status = DAQ_VScale (DEVICE_AT_MIO_16, CHANNEL_0, dltsDoc->getGain(), GAIN_ADJUSTMENT, SCALE_OFFSET, 1999, dataArrayAverage, voltageArray);
			if (status != 0){	 
