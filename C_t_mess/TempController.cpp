...
static const float DEVIANCE_MAX = 1.0;
static const float ERROR_VALUE = -9999999;
static const float ABORT_VALUE = -9999990;
static const int CHECK_INTERVAL = 10; // 10 seconds
...
float TempController::setTemperature(float Tsetpoint, bool bWaitForTemp){
	...
	while (true){					
		...
		bool bContinue = wait(CHECK_INTERVAL);
		if (!bContinue){
			return ABORT_VALUE;
		}

		if (hasTemperature(Tcurrent, Tsetpoint)){
			break;
		}
	}
	return Tcurrent;
...
void TempController::start(int TStart, int TEnd, int TStep, int holdTime){
	...
	while ( Tsetpoint <= TEnd){
		Tcurrent = this->setTemperature(Tsetpoint);
