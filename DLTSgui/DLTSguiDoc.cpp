...
BEGIN_MESSAGE_MAP(CDLTSguiDoc, CDocument)
	...
	ON_COMMAND(ID_FILE_EXPORTDLTSSPECTRUM, OnFileExportDLTSSpectrum)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
CDLTSguiDoc::CDLTSguiDoc()
{
	...
	m_NumberOfCapacitanceValues = DEFAULT_NUMBER_OF_SAMPLES;
}
...
BOOL CDLTSguiDoc::OnNewDocument()
{
	...
	//m_NumberOfCapacitanceValues = pApp->GetProfileInt("DLTS Settings","numberOfCapacitanceValues", 2000);
...
BOOL CDLTSguiDoc::OnSaveDocument(LPCTSTR lpszPathName) 
{
	...
	//pApp->WriteProfileInt("DLTS Settings","numberOfCapacitanceValues", m_NumberOfCapacitanceValues);
...
// the implementation is WRONG!!! Do not use any more...
double CDLTSguiDoc::LockIn(CDataArray *dataArray)
{
	double valueT1 = 0;
	double valueT2 = 0;
	int count = 0;
...
void CDLTSguiDoc::OnFileExportDLTSSpectrum() 
{
	CFileDialog *dialog;
	dialog = new CFileDialog(FALSE, NULL, NULL, OFN_HIDEREADONLY, "csv Dateien (*.csv)|*.csv|");
	if (dialog->DoModal() == IDOK){
		CString sFileName;
		CFile file;
		sFileName = dialog->GetPathName();
		...
		while (m_dataArrayList->HasNext()){
			...
			float value = 0;
			if (m_bDoubleBoxCar){
				value = DoubleBoxCar(dataArray);
			} else {
				value = LockIn(dataArray);
			}
			float temperature = dataArray->GetTemperature() + 273;
			CString str;
			str.Format("%3.1f, %3.5f\r\n", temperature, value);
			file.Write(str.GetBuffer(str.GetLength()), str.GetLength());
		}
	}
