...
void CDLTSguiView::DrawLegendDlts(CDC *pDC, CRect &rect)
{
	if (GetDocument()->m_pSelectedDataArray != NULL){
		...
		double temperature = 273.0 - GetDocument()->m_pSelectedDataArray->GetTemperature();
		strTemp.Format("dC(%3.1fK): %3.3fpF", temperature, value);
		pDC->TextOut(rect.left, rect.top - 20, strTemp);
		UpdateData(FALSE);
	}
...
void CDLTSguiView::DrawGraphDlts(CDC* pDC, CRect &rect)
{
	...
	while (GetDocument()->GetDataArrayList()->HasNext()){
		...
		double value = 0;
		if (GetDocument()->m_bDoubleBoxCar){
			value = GetDocument()->DoubleBoxCar(dataArray);
		} else {
			value = GetDocument()->LockIn(dataArray);
		}
...
void CDLTSguiView::OnRButtonDown(UINT nFlags, CPoint point) 
{
	...
	menu.AppendMenu(MF_STRING, ID_VISIBLE_AREA, "Graph Range");
	menu.AppendMenu(MF_STRING | MF_SEPARATOR , 0, "Graph");
...
//	menu.AppendMenu(MF_STRING, ID_VISIBLE_AREA_RESET_ALL, "reset range (all views)");
	...
	if (m_bShowDltsDiagram){
		menu.AppendMenu(MF_STRING | MF_CHECKED, ID_GRAPH_DLTS, "DLTS Spectrum");
	} else {
		menu.AppendMenu(MF_STRING, ID_GRAPH_DLTS, "DLTS Spectrum");
	}
	...
	if (!m_bShowDltsDiagram){
		...
		menu.AppendMenu(MF_STRING | MF_GRAYED, ID_GRAPH_LOKIN, "Time Rate Integration");
	} else {
		if (GetDocument()->m_bDoubleBoxCar){
			menu.AppendMenu(MF_STRING | MF_CHECKED, ID_GRAPH_DOUBLE_BC, "Double Boxcar");
			menu.AppendMenu(MF_STRING, ID_GRAPH_LOKIN, "Time Rate Integration");
		} else {
			menu.AppendMenu(MF_STRING, ID_GRAPH_DOUBLE_BC, "Double Boxcar");
			menu.AppendMenu(MF_STRING | MF_CHECKED, ID_GRAPH_LOKIN, "Time Rate Integration");
		}
	}
...
BOOL CDLTSguiView::ShowProperties()
{
	...
	parameters.m_recordingTime = GetDocument()->m_NumberOfCapacitanceValues / 100;
	...
	if (parameters.DoModal() == IDOK){
		...
		GetDocument()->m_NumberOfCapacitanceValues = parameters.m_recordingTime * 100;
		...
		//pApp->WriteProfileInt("DLTS Settings","numberOfCapacitanceValues", GetDocument()->m_NumberOfCapacitanceValues);
...
{
	if (GetDocument()->m_bDoubleBoxCar){
		GetDocument()->m_bDoubleBoxCar = false;
	} else {
		GetDocument()->m_bDoubleBoxCar = true;
	}
	Invalidate(TRUE);
...
void CDLTSguiView::OnUpdateGraphDoubleBc(CCmdUI* pCmdUI) 
{
	...
	pCmdUI->SetCheck(GetDocument()->m_bDoubleBoxCar);
